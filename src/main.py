...
def alexa_file_recognizer_generate_data(filepath, boundary):
	"""
	Generate a iterator for chunked transfer-encoding request of Alexa Voice Service
	Args:
		audio: raw 16 bit LSB audio data
	...
	chunksize = 1024
	startASKaudio = wave.open(filepath, "rb")

	riffdata = startASKaudio.readframes(chunksize)
	if(riffdata):
		yield bytes(riffdata)
	while riffdata:
		riffdata = startASKaudio.readframes(chunksize)
		yield bytes(riffdata)
...
def alexa_file_recognizer(filepath):
	# https://developer.amazon.com/public/solutions/alexa/alexa-voice-service/rest/speechrecognizer-requests
	...
	data = alexa_file_recognizer_generate_data(filepath, boundary)
	resp = requests.post(url, headers=headers, data=data)
...
def trigger_process(trigger):
	...
	if trigger.voice_confirm:
		...
		alexa_file_recognizer(resources_path + 'start.wav')
	#else:
